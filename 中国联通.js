...
4. 特殊功能设置:
   export ONLY_AYTING=true  : (已废弃，无需单独补跑)
...
!(async () => {
  ...
  // ==========================================
  // 纯串行模式：获取IP -> 登录 -> 任务 -> 下一个
  // ==========================================
  appName.log(`🚀 开始串行执行日常任务...`);

  for (let user of appName.userList) {
    // 1. 获取代理IP并登录 (userLoginTask内部包含了 set_proxy_ip 和 onLine)
    // 只有在轮到该账号时才请求IP，最大程度节省资源
    appName.log(`\n🔄 正在初始化账号[${user.index}]...`);
